...
namespace DiscordBot.Modules.UrbanDictionary
{
    public class UrbanDictionaryCommand : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task ExecuteAsync([Remainder][Summary("A phrase")] string phrase)
        {
            ...
            try
            {
                ...
                if (result?.List is null || result?.List.Count == 0)
                {
            ...
            {
                await ReplyAsync("Error maikng the request to Urban Dictionary API");
            }
            catch (Exception ex)
            {
                await ReplyAsync($"An error occured: {ex.Message}");
            }
        }
    }
